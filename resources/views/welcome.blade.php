                ...
                @foreach($products as $product)
                    <div class="bg-white rounded-lg shadow-md overflow-hidden relative group border-t-4 border-red-600">
                        <a href="{{ route('products.show', $product->id) }}">
                            <img src="{{ $product->image_url }}" alt="{{ $product->name }}" class="w-full h-64 object-cover group-hover:scale-105 transition-transform duration-300">
                        </a>
                        <div class="p-4">
                            <a href="{{ route('products.show', $product->id) }}">
                                ...
                                @php
                                    ...
                                    if ($product->category) {
                                        ...
                                        while ($cat) {
                                            ...
                                            if ($slug === 'peinture' || $slug === 'peintures') { $isPaintCategory = true; break; }
                                            $cat = $cat->parent ?? null;
                                        }
                                    }
                                    ...
                                    if ($minPromoPrice && $minPromoPrice < $minPrice) { $hasPromotion = true; }
                                @endphp

                                @if($isPaintCategory)
                                    <span class="text-sm font-medium text-gray-500">Prix sur devis</span>
                                @else
                                    @if($hasPromotion)
